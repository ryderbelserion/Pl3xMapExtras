...
public class PaperUserAdapter extends IUser {
    ...
    public @NotNull final UUID getUniqueId() {
        return this.player == null ? Pl3xMapPlugin.CONSOLE_UUID : this.player.getUniqueId();
    }

    @Override
    public @NotNull final String getUsername() {
        return this.player == null ? Pl3xMapPlugin.CONSOLE_NAME : this.player.getName();
    }
    ...
    public void setLocale(@NotNull final String locale) {
        ...
        if (!value.equalsIgnoreCase("en_us.yml")) {
            this.locale = Key.key(Pl3xMapPlugin.namespace, value);
        }
    }
}