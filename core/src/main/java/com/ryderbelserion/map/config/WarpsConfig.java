...
public abstract class WarpsConfig extends AbstractConfig {
    ...
    private static final Pl3xMapPlugin platform = (Pl3xMapPlugin) Pl3xMapExtras.Provider.getInstance();

    private static final BasicConfig config = platform.getBasicConfig();
    ...
    public static void registerIcon(@NotNull final String image) {
        if (!config.isWarpsEnabled()) return;
